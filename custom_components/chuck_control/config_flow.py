...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def async_get_options_flow(
        ...
        return ChuckOptionsFlowHandler(config_entry)
    ...
    async def async_step_user(
        ...
        try:
            await chuckapi.update()  # Corrected line: Await directly
...
class ChuckOptionsFlowHandler(config_entries.OptionsFlow):
    """Handle options flow for Chuck Charger Control."""
    ...
    async def async_step_init(
        ...
        self.data_schema = vol.Schema(
            {
                vol.Required(
                    "friendly_name", default=self.config_entry.options.get("friendly_name")
                ): cv.string,
                vol.Required("base_url", default=self.config_entry.options.get("base_url")): cv.string,
                vol.Required("auth_user", default=self.config_entry.options.get("auth_user")): cv.string,
                vol.Required("auth_pass", default=self.config_entry.options.get("auth_pass")): cv.string,
                vol.Required(
                    "cfg_phase_order_conn1",
                    default=self.config_entry.options.get("cfg_phase_order_conn1"),
                ): vol.In(PHASE_ORDER_DICT),
                vol.Required(
                    "cfg_phase_order_conn2",
                    default=self.config_entry.options.get("cfg_phase_order_conn2"),
                ): vol.In(PHASE_ORDER_DICT),
                vol.Optional(
                    "have_net_current_sensor",
                    default=self.config_entry.options.get("have_net_current_sensor"),
                ): cv.boolean,
                vol.Optional(
                    "is_connected_to_ocpp",
                    default=self.config_entry.options.get("is_connected_to_ocpp"),
                ): cv.boolean,
            }
        )
        return self.async_show_form(
